...
setter_list = ["김다인","김다솔","안혜진","이다영","이원정","이나연","조송화","김하경","하효림","염혜선","이고은"]
        ...
        for idx in range(len(text_list)):
            ...
            if(home_txt.find("세트")> -1):
                if(home_txt[-2:]=="성공"):
                    ...
                    temp = [home_team,away_team,setter,home_players[2],home_players[5],home_players[4],home_players[3],home_players[0],home_players[1],op_name,home_score,away_score,home_score-away_score]
                    lst.append(temp)
                elif(home_txt[-2:]=="세트"):
                    fail = fail + 1
                    setter = home_txt[home_txt.find(".")+1:home_txt.find(" ")]
                ...
                if(away_txt[-2:]=="성공"):
                    ...
                    op_type = op_txt[op_txt.find(" ")+1:op_txt.find("성")]
                    temp = [away_team,home_team,setter,away_players[3],away_players[0],away_players[1],away_players[2],away_players[5],away_players[4],op_name,away_score,home_score,away_score-home_score]
                    lst.append(temp)
                elif(away_txt[-2:]=="세트"):
                    fail = fail + 1
                    setter = away_txt[away_txt.find(".")+1:away_txt.find(" ")]
...
df1 = pd.DataFrame(data = np.array(lst),columns=["팀","상대팀","세터","1번","2번","3번","4번","5번","6번","공격수","팀 점수","상대팀 점수","점수차이"])
df1.to_csv("/Users/kimjungwoo/Downloads/setter.csv")
